...
async def base_interpret(
    ...
    # --- Step 5: LLM call (only on warn/alert) ---
    ...
    try:
        ...
        llm_severity, summary, reasoning = _parse_llm_response(raw)
        # LLM can upgrade severity but not downgrade below heuristic floor
        severity = max(severity, llm_severity, key=lambda s: list(Severity).index(s))
    except Exception as e:
        logger.error("LLM evaluation failed: %s", e)
        summary = f"LLM error fallback: {agg_signals.worst_signal()}"
        reasoning = str(e)
