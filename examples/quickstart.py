...
def make_dataset(n: int = 1000, drift: bool = False) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "description":  [
            "active user high engagement"   if not drift else
            "churned low activity dropout"   # token distribution shift
            for _ in range(n)
        ],
...
async def main():
    ...
    df_results = storage.query(
        "SELECT job_id, severity, summary, jaccard_delta "
        "FROM interpretations ORDER BY run_ts"
    ).df()
